...
def main():
    ...
    # Generate randoms AppVar (shared)
    randoms_path = generate_randoms_appvar(convbin, output_dir)

    # Generate book data AppVars
    book_paths = generate_book_appvars(convbin, input_bin, output_dir, tier)

    # Bundle into .8xg group
    all_paths = [randoms_path] + book_paths
    print()
    print("Bundling:")
    generate_group(convbin, all_paths, output_dir, tier)

    print()
    print(f"Total files: {len(all_paths)} AppVars + 1 group")
    print(f"Transfer to calculator: either send individual .8xv files,")
    print(f"  or send the .8xg group (auto-extracts on calculator).")
